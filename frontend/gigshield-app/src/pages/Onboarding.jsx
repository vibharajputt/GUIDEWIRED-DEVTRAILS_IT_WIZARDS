          ...
          <div className="flex justify-between mb-4 px-2 relative z-10">
            {['Identify', 'Details', 'Coverage', 'Secure'].map((label, i) => (
              <div key={i} className={`text-[10px] uppercase tracking-widest font-bold flex flex-col items-center flex-1 transition-colors duration-300 ${Math.ceil(step) >= i + 1 ? 'text-brand-300' : 'text-slate-600'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center mb-2 border-2 transition-all duration-500 z-10 relative ${Math.ceil(step) > i + 1 ? 'bg-brand-500 border-brand-500 text-white' :
              ...
              <div className="border border-brand-500/30 rounded-2xl p-5 bg-brand-900/10 mt-6 relative overflow-hidden">
                ...
                <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3 relative z-10">
                  <input placeholder="e.g., test@upi" value={formData.upi_id}
                    ...
                    className={`flex-1 px-4 py-3 rounded-xl border focus:outline-none shadow-inner transition ${upiVerified ? 'border-emerald-500/50 bg-emerald-900/20 text-emerald-100' :
                  ...
                  <button onClick={handleVerifyUPI} disabled={!formData.upi_id || upiVerifying || upiVerified}
                    className={`px-6 py-3 rounded-xl font-bold text-sm whitespace-nowrap transition shadow-lg ${upiVerified
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
            ...
            <div className="space-y-4">
              {['basic', 'standard', 'pro'].map((planType) => {
                ...
                return (
                  <div key={planType} className={`glass-panel p-6 sm:p-8 rounded-[2rem] shadow-lg border-2 transition-all relative overflow-hidden ${isRecommended ? 'border-brand-500 shadow-[0_0_30px_rgba(79,70,229,0.15)] transform hover:-translate-y-1' : 'border-white/5 hover:border-white/20'
                    }`}>
                    {isRecommended && (
                    ...
                    <button onClick={() => handleCreatePolicy(planType)} disabled={loading}
                      className={`w-full flex justify-center items-center py-3.5 rounded-xl font-bold transition-all relative z-10 ${isRecommended
